...
class CovidBert(nn.Module):
    def __init__(self):
        super(CovidBert, self).__init__()
        if args.pretrained_model_name == "seiya/oubiobert-base-uncased":
            self.bert = AutoModelForPreTraining.from_pretrained(args.pretrained_model_name,output_hidden_states=True)
        else:
            self.bert = AutoModel.from_pretrained(args.pretrained_model_name,output_hidden_states=True)
...
class CovidBertNext(nn.Module):
    def __init__(self):
        super(CovidBertNext, self).__init__()
        if args.pretrained_model_name == "seiya/oubiobert-base-uncased":
            self.bert = AutoModelForPreTraining.from_pretrained(args.pretrained_model_name,output_hidden_states=True)
        else:
            self.bert = AutoModel.from_pretrained(args.pretrained_model_name,output_hidden_states=True)
